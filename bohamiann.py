...
from utilities.priors import weight_prior, log_variance_prior
from utilities.sampler import AdaptiveSGHMC, SGLD, SGHMC, PreconditionedSGLD
from utilities.infinite_dataloader import infinite_dataloader
from utilities.layers import AppendLayer
from utilities.normalization import zero_mean_unit_var_denormalization, zero_mean_unit_var_normalization


def get_default_network(input_dimensionality: int, n_units =[50, 50, 50]) -> torch.nn.Module:
    class Architecture(torch.nn.Module):
        def __init__(self, n_inputs, n_hidden=n_units):
            super(Architecture, self).__init__()
            self.fc1 = torch.nn.Linear(n_inputs, n_hidden[0])
...
class Bohamiann(BaseModel):
    ...
    def train(self, x_train: np.ndarray, y_train: np.ndarray,
        ...
        if not continue_training:
            ...
            if self.use_double_precision:
                self.model = self.get_network(input_dimensionality=input_dimensionality, n_units = self.n_units).double()
            else:
                self.model = self.get_network(input_dimensionality=input_dimensionality, n_units = self.n_units).float()
    ...
    def predict(self, x_test: np.ndarray, return_individual_predictions: bool = False):
        ...
        print(f'n_sample_used={len(self.sampled_weights)}')
